...
import torch
from data_helpers.data_helper_v2 import CEG4EVAL
import json
...
with torch.no_grad():
    for index,batch in enumerate(tqdm(val_data)):
        ...
        if args.model_type == 'glm3' or args.model_type == 'glm4':
            generated_ids = model.generate(**batch_input, max_length=batch_input['input_ids'].shape[-1] + args.max_target_length,num_beams=1)
