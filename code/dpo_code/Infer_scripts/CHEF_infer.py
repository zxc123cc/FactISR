...
device = "cuda"
tokenizer = AutoTokenizer.from_pretrained("/mnt/user/luyifei/model_weight/glm4_CHEF_lora_sft",trust_remote_code=True)
model = AutoModelForCausalLM.from_pretrained(
    "/mnt/user/luyifei/model_weight/glm4_CHEF_lora_sft",
    torch_dtype=torch.bfloat16,
    low_cpu_mem_usage=True,
    trust_remote_code=True
).to(device).eval()
# CHEF_Data
with open('/mnt/user/luyifei/cs_data/final_data/final_data/CHEF/test.json' ,'r') as f:
    CHEF_test = json.load(f)
res = {}
for index in tqdm(range(len(CHEF_test))):
    claim = CHEF_test[index]['claim']
    if 'text' not in CHEF_test[index]['evidence']:
        evidence_list = CHEF_test[index]['evidence']
        evidence = convert_list_to_str(evidence_list)
    else:
        evidence = CHEF_test[index]['evidence']['text']
    ...
    # break
with open('/mnt/user/luyifei/cs_data/CHEF_dpo_infer.json', 'w', encoding='utf-8') as f:
