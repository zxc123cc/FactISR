...
class CEGDataSet(Dataset):
    ...
    def __init__(self, data, tokenizer, max_source_length, max_target_length):
        ...
        self.self_correction_prompt = ("你是一个事实核查领域解释修正的专家，擅长根据证据从多个角度修正给定的解释（此处的解释是根据证据来表明为何当前说法是正确的/错误的/证据不足或不充分的）。"
                                       "我将提供当前说法、相应的证据以及当前的解释，你要根据证据，将这段解释进行修正，要求如下。"
                                       "要求1：使修正后的解释更加精炼，不要包含过多冗余信息。"
                                       "要求2：忠于证据，可以多多引用证据中的话。"
                                       "要求3：将真实性的错误改正（有些解释可能存在真实性错误，如当前解释判断该说法是错误的，而你可以非常肯定的基于证据判断当前说法是正确的，那么就将其纠正）"
                                       "当前说法为：{}"
                                       "对应证据为：{}"
                                       "该说法的解释为{}"
                                       "请你生成修正后解释：")
    ...
    def get_data(self, data):
        ...
        for index in range(len(data)):
            ...
            evidence_str = self.get_truncation_text(evidence_str, tranc_len=self.max_source_length - 500)  # Reserve 100 length for instruction

            fact_verification_data = data[index].copy()
            explanation_generation_data = data[index].copy()
            self_correction_data = data[index].copy()
            ...
            pseudo_explanation = data[index]['pseudo_explanation']
            evidence_str = self.get_truncation_text(evidence_str, tranc_len=self.max_source_length - 1000)
            self_correction_input = self.self_correction_prompt.format(claim, evidence_str, pseudo_explanation)
            explanation = data[index]['human_revision']
            self_correction_data['input'] = self_correction_input
            self_correction_data['label'] = explanation
            my_data.append(self_correction_data)
    ...
    def get_data(self, data):
        ...
        for index in range(len(data)):
            ...
            if self.task_mode == 'FVP':
                ...
                explanation_generation_input = self.explanation_generation_prompt.format(claim, evidence_str, pseudo_fact_label)
                explanation_generation_data['input'] = explanation_generation_input
                my_data.append(explanation_generation_data)
            else:
                self_correction_data = data[index].copy()
