...
from data_helpers.data_helper_v4 import CEGDataSet, DataCollatorForCEG
...
def main():
    ...
    with open('../test_result/stage1/Qwen2_stage1_train_ep5_lr3e-4/EGP_train_all.json', "r", encoding="utf-8") as f:
        pseudo_explanation_data = json.load(f)

    for idx, data in enumerate(train_data):
        data["pseudo_explanation"] = pseudo_explanation_data[str(idx)]

    train_dataset = CEGDataSet(
        train_data,
        tokenizer,
    ...
    peft_config = LoraConfig(
        ...
        target_modules=["q_proj", "k_proj", "v_proj", "o_proj", "gate_proj", "up_proj","down_proj"], #qwen2
    ...
    data_collator = DataCollatorForCEG(
        tokenizer,
        model=model,
